...
- **Dynamo cannot trace `ContextVar.get`/`set`**: state read or written inside
  torch.compile-traced model code (piecewise cuda graph compiles the whole model
  forward) must be plain-slot backed — see `ForwardFlags._GRAPH_VISIBLE`. Before
  moving any such state onto a contextvar, prove its readers sit outside compile
  coverage; a piecewise-prefill boot of any small model is the fast check.
